...
def update_username_list(username):
    folder_path = os.path.join(os.path.dirname(__file__), "wordlists")
    file_path = os.path.join(folder_path, "commondutchwords.txt")
...
class Worker(Thread):
    ...
    def get_headers(self, token):
        headers = {
            ...
            "accept": "*/*",
            "accept-language": "en;q=0.9,nl;q=0.8,en-GB;q=0.7,en-US;q=0.6",
        ...
        while not self.usernames_queue.empty():
            with self.lock:
                ...
                if best_token is None:
                    logger.warning(
                        f"Thread {self.name}: No available tokens. Exiting thread."
                    )
                    return
            ...
            if result == "taken":
                ...
                append_to_file("unavailable.txt", username)
                ...
                append_to_file("available.txt", username)
                best_token.set_sleep_until(time.time() + random.uniform(4, 6))
            elif result == "connection_error":
                logger.warning(
                    f"Thread {self.name}: Connection error. Retrying after 10 seconds."
                )
...
def main():
    ...
    tokens, usernames = load_file("tokens.txt"), load_file(
        "wordlists/commondutchwords.txt"  ## update this to your own wordlist
    )
    if not usernames:
        logger.error("commondutchwords.txt file is empty.")
        input()
        return
    if not tokens:
        logger.error("tokens.txt file is empty.")
        input()
        return
    valid_tokens = []
    for token in tokens:
        ...
        while connection_error:
            try:
                ...
                if "retry_after" in response_json:
                    ...
                    logger.success(f"Token {token} is ready for use.")
                ...
                logger.warning(
                    f"Connection error while checking token: {token}. Retrying after 10 seconds."
                )
                time.sleep(10)
    if not valid_tokens:
        logger.error("Operation aborted. No valid tokens.")
        input()
        return
    ...
    for worker in workers:
        worker.join()
    logger.success("Operation completed. No more usernames to check.")
    input()
